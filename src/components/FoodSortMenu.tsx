...
import { isNutrientKey } from "../lib/nutrients";
import { NutrientLabel } from "./NutrientLabel";

type Props = {
  sort: FoodSortState;
  onChange: (sort: FoodSortState) => void;
};

export function FoodSortMenu({ sort, onChange }: Props) {
  ...
  const selectField = (field: FoodSortField) => {
    if (sort.field === field) {
      onChange({
        field,
      ...
      const option = FOOD_SORT_OPTIONS.find((o) => o.field === field);
      onChange({
        field,
        direction: option?.defaultDirection ?? DEFAULT_FOOD_SORT.direction,
      });
    }
    setOpen(false);
  };

  const activeLabel = getSortOptionLabel(sort.field);
  const activeDirection = sortDirectionLabel(sort.field, sort.direction);
  ...
  return (
    <div ref={containerRef} className="relative shrink-0">
      <button
        ...
        aria-expanded={open}
        aria-label={`Sort foods: ${activeLabel}, ${activeDirection}`}
      ...
      {open && (
        ...
        >
          {FOOD_SORT_OPTIONS.map(({ field, label }) => {
            const active = sort.field === field;
            const directionLabel = active
              ? sortDirectionLabel(field, sort.direction)
              : null;
